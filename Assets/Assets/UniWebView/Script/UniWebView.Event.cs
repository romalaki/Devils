...
public partial class UniWebView {
    ...
    /// <summary>
    /// Delegate for popup window opening event.
    /// </summary>
    /// <param name="webView">The web view component which opens the popup window.</param>
    /// <param name="popup">The popup window handle.</param>
    public delegate void PopupWindowOpenedDelegate(UniWebView webView, UniWebViewPopup popup);
    /// <summary>
    /// Raised when a popup window is opened. This provides a lightweight popup handle.
    /// </summary>
    public event PopupWindowOpenedDelegate OnPopupWindowOpened;
    ...
    /// <summary>
    /// Delegate for popup window closing event.
    /// </summary>
    /// <param name="webView">The web view component which closes the popup window.</param>
    /// <param name="popup">The popup window handle.</param>
    public delegate void PopupWindowClosedDelegate(UniWebView webView, UniWebViewPopup popup);
    /// <summary>
    /// Raised when a popup window is closed.
    /// </summary>
    ...
    internal void InternalOnMultipleWindowOpened(string multiWindowId) {
        var popup = GetOrCreatePopupWindow(multiWindowId);
        if (OnMultipleWindowOpened != null) {
            OnMultipleWindowOpened(this, multiWindowId);
        }
        if (OnPopupWindowOpened != null) {
            OnPopupWindowOpened(this, popup);
        }
    }

    internal void InternalOnMultipleWindowClosed(string multiWindowId) {
        HandlePopupWindowClosed(multiWindowId);
    }
