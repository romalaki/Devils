...
public partial class UniWebView {
    ...
    /// This API is deprecated and no longer grants SSL exceptions.
    /// Use <see cref="AddSslPinnedFingerprint"/> instead.
    /// </summary>
    /// <param name="domain">The domain to add. It should not contain any scheme or path part in url.</param>
    [Obsolete("AddSslExceptionDomain is deprecated. Use AddSslPinnedFingerprint instead.", true)]
    ...
    /// This API is deprecated and kept for binary compatibility only.
    /// </summary>
    /// <param name="domain">The domain to remove. It should not contain any scheme or path part in url.</param>
    [Obsolete("RemoveSslExceptionDomain is deprecated. Use RemoveSslPinnedFingerprint instead.", true)]
    ...
    /// <summary>
    /// Pins a certificate fingerprint for a domain when handling SSL errors.
    /// Only when the presented certificate matches the SHA-256 fingerprint will the page be allowed to load.
    /// </summary>
    /// <param name="domain">The domain to trust. It should not contain any scheme or path part.</param>
    /// <param name="fingerprint">The SHA-256 fingerprint of the expected certificate. Colons are optional.</param>
    public void AddSslPinnedFingerprint(string domain, string fingerprint) {
        if (string.IsNullOrEmpty(domain)) {
            UniWebViewLogger.Instance.Critical("The domain should not be null or empty.");
            return;
        }
        if (domain.Contains("://")) {
            UniWebViewLogger.Instance.Critical("The domain should not include invalid characters '://'");
        ...
        if (string.IsNullOrEmpty(fingerprint)) {
            UniWebViewLogger.Instance.Critical("The fingerprint should not be null or empty.");
            return;
        }
        UniWebViewInterface.AddSslPinnedFingerprint(listener.Name, domain, fingerprint);
    }

    /// <summary>
    /// Removes a previously pinned certificate fingerprint for a domain.
    /// </summary>
    /// <param name="domain">The domain whose fingerprint should be removed.</param>
    /// <param name="fingerprint">The SHA-256 fingerprint registered earlier.</param>
    public void RemoveSslPinnedFingerprint(string domain, string fingerprint) {
        if (string.IsNullOrEmpty(domain)) {
            UniWebViewLogger.Instance.Critical("The domain should not be null or empty.");
            return;
        }
        ...
        UniWebViewInterface.RemoveSslPinnedFingerprint(listener.Name, domain, fingerprint);
    }
    ...
    /// <summary>
    /// Enables or disables the automatic download when the web view receives a response that cannot be rendered inline.
    ///
    /// When set to `false`, UniWebView stops starting a download task for non-renderable resources (such as attachments
    /// with an unsupported MIME type). On iOS and macOS Editor, downloads triggered by `AddDownloadURL` or
    /// `AddDownloadMIMEType` still work. On Android, this controls downloads started by the system download listener.
    /// Default is `true`.
    /// </summary>
    /// <param name="enabled">Whether UniWebView should automatically start downloading non-renderable resources.</param>
    public void SetAutoDownloadEnabled(bool enabled) {
        #if UNITY_ANDROID && !UNITY_EDITOR
        UniWebViewInterface.SetAutoDownloadEnabled(listener.Name, enabled);
        #elif (UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX || UNITY_IOS) && !UNITY_EDITOR_WIN && !UNITY_EDITOR_LINUX
        UniWebViewInterface.SetAutoDownloadEnabled(listener.Name, enabled);
        #endif
    }
